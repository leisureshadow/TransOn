...
public class UpdateHistoryInfoPacketHandler extends PacketHandler {

	public UpdateHistoryInfoPacketHandler(ServiceHandler _serviceHandler) {
		super(_serviceHandler);
	}

	@Override
	protected boolean isResponsible(Packet packet) {
		return packet.getCommand().compareTo("UpdateHistoryInfo") == 0;
	}

	@Override
	protected void handle(Packet packet) {
		MeetingInformation meetingInformation = (MeetingInformation)packet.getItem("historyInfo", MeetingInformation.class);
		HistoryRoom historyRoom = ((HistoryServiceHandler)serviceHandler).getHistoryRoom();
		historyRoom.updateHistoryInfo(meetingInformation);
	}
